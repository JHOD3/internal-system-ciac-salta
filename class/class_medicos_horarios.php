...
class Medicos_horarios extends Estructura implements iMedicos_horarios{
	...
	function FormAlta($id_padre){
		...
		requerir_class("turnos_tipos", "dias_semana", "plantas");
		...
		$htm->Asigna("DROP_PLANTA", utf8_encode($obj_plantas->Drop()));
	...
	function FormModificacion(){
		...
		$obj_plantas = new Plantas();
		$htm->Asigna("DROP_PLANTA", utf8_encode($obj_plantas->Drop("DESC",$row['id_plantas'])));
