...
class Cobros extends Estructura implements iCobros{
	
	function __construct($id = ""){ 
		$this->nombre_tabla = "cobros";
		$this->titulo_tabla = "Cobros";
		$this->tabla_padre = "pacientes";
		
		$this->drop_label_elija = "Elija un cobro";
	...
	function ReintegroEfectuado(){
		$query_string = $this->querys->ReintegroEfectuado($this->id);
	...
	function BajaCobrosxMedicos($id_turno){
		$query_string = $this->querys->BajaCobrosxMedicos($id_turno);
		if ($this->db->consulta($query_string))
			$rta = true;
		else
			$rta = false;
