...
class Medicos extends Estructura implements iMedicos{
	...
	function __construct($id = ""){ 
		...
		requerir_class("pacientes", "turnos_estados", 'sectores', "tipos_documentos");
	} 
	...
	function Buscar($termino){
		...
		if ($cant != 0){
			while ($row = $this->db->fetch_array($query)){
				$elto = Array("id"=>$row["id_medicos"], "nombres"=>$row["nombres"], "apellidos"=>$row["apellidos"], "icon" => $row["email"]);
				$rta[] = $elto;		
			}
		}
	...
	function GrillaTurnos($id_medico, $id_especialidad, $id_dia, $fecha){
		...
		if ($fecha >= date('y-m-d')){
			...
			if ($cant != 0){
				while ($row = $this->db->fetch_array($query)){
					$row["PACIENTE"] =$row ["apellidos"].", ".$row['nombres'];
					$row['USUARIO'] = $row["USR_NOMBRE"]." ".$row["USR_APELLIDO"];
					$obj_turno_estado = new Turnos_estados($row["id_turnos_estados"]);
					$row["ESTADO"] = $obj_turno_estado->nombre;
					$htm->AsignaBloque('block_registros',$row);
				}
	...
	function GrillaTurnosEstetica($id_medico, $id_especialidad, $id_dia, $fecha){
		...
		if ($fecha >= date('y-m-d')){
			...
			if ($cant > 0){
				...
				//PREGUNTO SI EXISTEN TURNOS RESERVADOS
				if ($cant2 > 0){
					$i = 1;
					while ($row = $this->db->fetch_array($query2)){
						...
						switch ($_SESSION['SISTEMA']){
							case 'sas':
								$linea = " <span style='color:#".$row["color"]."' class='btn_estado_turno' data-id='".$row["id_turnos"]."' data-id_turnos_tipos='".$row["id_turnos_tipos"]."' data-id_turnos_estados='".$row["id_turnos_estados"]."' data-tipo='turno' data-consultorio='".$row['consultorio']."'>
									<div class='bloque'>
										<img src='".IMG."btns/tipo_".$tipo_turno.".png' /><span>".$row["desde"].": <br />
<small>Consul. ".$row['consultorio']."</small></span> 
										<div class='dat_paciente'>".
											$row["apellidos"]. ", ".$row["nombres"]." 
											(".$row["nombre_estado"].")<br />
											<small style='color:#000'>".$row["nombre_os"]. " - ".$row["telefonos"]."</small>
						...
						$grillav[$row["desde"].$row["consultorio"]] = $linea;
				...
				while ($row2 = $this->db->fetch_array($query)){
					...
					while ($suma <= $row2["hasta"]){
						...
						//PREGUNTO SI EXISTEN TURNOS LIBRES O SI SE DIERON LOS 3
						if ($cantidad < 3){
						/////////////////
							$es_inhabilitado = false;
							foreach ($inhabilitadosv as $clave => $valor){
								...
								switch ($_SESSION['SISTEMA']){
									case 'sas':
										...
										for ($i=0; $i < 3 - $cantidad; $i++){
											$consultorio = $this->ConsultorioReservado($grillav,$inicio,$i + 1);
											$linea_aux = "<span class='reservar libre' data-desde='".$inicio."' data-hasta='".$fin."' data-fecha='".$fecha."' data-turnos_tipos='".$tipo_turno."' data-consultorio='".$consultorio."'>
												<div class='bloque'>
													<img src='".IMG."btns/tipo_".$tipo_turno.".png' />".$inicio." <small>Consul. ".$consultorio."</small>: <strong>Libre</strong>
												</div>
											
											</span>";
											
											
											$grillav[$inicio.$consultorio] = $linea_aux;
										}
									break;
									case 'sam':
										$linea = "<span href='#' class='reservar libre' data-desde='".$inicio."' data-hasta='".$fin."' data-fecha='".$fecha."' data-turnos_tipos='".$tipo_turno."'>
