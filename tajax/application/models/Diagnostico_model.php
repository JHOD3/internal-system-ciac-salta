...
class Diagnostico_model extends CI_model
{
    ...
    public function obtEspecialidadParaDiagnostico($id_especialidades)
    {
        $query = $this->db
            ->from('especialidades')
    ...
    public function obtEspecialidadesParaDiagnostico()
    {
        $query = $this->db
    ...
    public function obtMedicoParaDiagnostico($id_medicos)
    {
        $query = $this->db
            ->from('medicos')
    ...
    public function obtObrasSocialesDeMedicoParaDiagnostico($id_medicos)
    {
        $query = $this->db
    ...
    public function obtObrasSocialesParaDiagnostico()
    {
        $query = $this->db
            ->from('obras_sociales')
            ->where('estado', 1)
    ...
    public function obtObrasSocialesDropDownMedicoParaDiagnostico($id_medicos)
    {
        $query = $this->db
            ->from('obras_sociales')
            ->where('estado', 1)
    ...
    public function obtObraSocialParaDiagnostico($id_obras_sociales)
    {
        $query = $this->db
            ->from('obras_sociales')
    ...
    public function obtMedicosParaDiagnostico($id_especialidades)
    {
        $query = $this->db
            ->select('m.*')
            ->from('medicos AS m')
    ...
    public function obtMedicosPorEspecialidadParaDiagnostico($tipo = '', $filter = '')
    {
        $this->db
            ->select('m.*, e.*')
            ->from('medicos AS m')
            ...
            ->where('e.estado', 1)
            ->where('tt.estado', 1)
            ->where_in('tt.id_turnos_tipos', array(9, 10))
...
SQL;
    ...
    public function obtDiasSemanaParaDiagnostico(
        $id_especialidades,
        $id_medicos,
        $year,
        $month
        ...
        $query = $this->db
            ...
            ->where('id_medicos', $id_medicos)
            ->where('estado', 1)
            ->where_in('id_turnos_tipos', array(9, 10))
    ...
    public function obtDiasParaDiagnostico($id_especialidades, $id_medicos, $year, $month){
        $proximoTurnoDisponible = $this->proximoTurnoDisponibleParaDiagnostico(
            $id_especialidades,
            $id_medicos
        );
        $dias_semana = $this->obtDiasSemanaParaDiagnostico(
            $id_especialidades,
            $id_medicos,
            $year,
            $month
        ...
        for ($i = 1; $i <= cal_days_in_month(CAL_GREGORIAN, $month, $year); $i++) {
            ...
            if (
                ...
                $result[$i] = base_url()."index.php/diagnostico/horarios/{$id_especialidades}/{$id_medicos}/{$year}/{$month}/".($i<10?'0':'')."{$i}/";
            }
        }
        return $result;
    }

    public function obtHorariosParaDiagnostico(
        $id_especialidades,
        $id_medicos,
        $year,
        $month,
    ...
    public function obtDiasHorariosParaDiagnostico($id_especialidades, $id_medicos, $year, $month){
        $query = $this->db
            ...
            ->where('mh.estado', 1)
            ->where_in('mh.id_turnos_tipos', array(9, 10))
    ...
    public function obtDuracionTurnoParaDiagnostico($id_especialidades, $id_medicos)
    {
        $query = $this->db
    ...
    public function obtTurnosReservadosParaDiagnostico(
        $id_especialidades,
        $id_medicos,
        $year,
        $month,
    ...
    public function obtTurnoParaDiagnostico($id_turnos) {
        $query = $this->db
            ->from('turnos AS t')
    ...
    public function obtHorariosInhabilitadosParaDiagnostico(
        $id_especialidades,
        $id_medicos,
        $year,
        $month,
    ...
    public function obtPacienteParaDiagnostico($nro_documento)
    {
        $query = $this->db
            ->from('pacientes')
    ...
    public function obtPacienteByIdParaDiagnostico($id_pacientes)
    {
        $query = $this->db
    ...
    public function addTurnoParaDiagnostico($post)
    {
        $this->db->trans_start();
            ...
            if (count($query) == 0) {
                ...
                $this->sendEmailTurnoReservadoParaDiagnostico($post, $this->db->insert_id());
            }
        $this->db->trans_complete();
        return true;
    }

    public function cancelTurnoParaDiagnostico($post)
    {
        // AGREGAR EL TURNO EN LA BD
        $this->db
    ...
    public function proximoTurnoDisponibleParaDiagnostico($id_especialidades, $id_medicos)
    {
        $day = date("Y-m-d");
        $total = 0;
        $max = 0;
        while ($total <= 0 and $max < 150) {
            ...
            $vcDT = $vcDuracionTurno = $this->obtDuracionTurnoParaDiagnostico(
                $id_especialidades,
                $id_medicos
            );
            $vcDT = explode(":", $vcDT);
            ...
            if ($total > 0) {
                $dados = $this->obtTurnosReservadosParaDiagnostico(
                    $id_especialidades,
                    $id_medicos,
    ...
    public function sendEmailTurnoReservadoParaDiagnostico($post, $id_turnos)
    {
        $post['id_turnos'] = $id_turnos;
        $aData['post'] = $post;
        $aData['rsE'] = $this->obtEspecialidadParaDiagnostico(
            $post['id_especialidades']
        );
        $aData['rsM'] = $this->obtMedicoParaDiagnostico(
            $post['id_medicos']
        );
        $aData['rsP'] = $this->obtPacienteByIdParaDiagnostico(
            $post['id_pacientes']
        );
        $to = $aData['rsP']['email'];
        ...
        $this->sendEmailParaDiagnostico($to, $subject, $message);
    }

    public function sendEmailParaDiagnostico($to, $subject, $message)
    {
        if ($to) {
    ...
    public function getTurnosNextParaDiagnostico($next)
    {
        $query = $this->db
            ->select(
    ...
    public function getTurnosCanceladosPorElMedicoParaDiagnostico($next)
    {
        $query = $this->db
            ->select('id_turnos')
