...
<?=form_open(base_url().$this->router->fetch_class().'/guardar_agregar');?>

    Fecha:
    <?=form_input('fecha', set_value('fecha'))?>
    <div class="validation_errors"><?php echo form_error('fecha'); ?></div>
    <br />

    Apellido:
    <?=form_input('apellido', set_value('apellido'))?>
    ...
    <select name="sexoid">
        ...
        <?php foreach($sexo_listado As $sexo_item):?>
            <option
                ...
                <?php if ($sexo_item['sexoid'] == set_value('sexoid')): ?>
                    selected="selected"
                <?php endif; ?>
            >
    ...
    Fecha de Nacimiento: <?=form_input('fechanac', set_value('fechanac'))?>
    ...
    Estado:
    <input type="radio" id="estado1" name="estado" value="1"<?=set_value_boolean('estado') != '0' ? ' checked="checked"' : ''?>/>
    <label for="estado1">Activo</label>
    <input type="radio" id="estado2" name="estado" value="0"<?=set_value_boolean('estado') == '0' ? ' checked="checked"' : ''?>/>
